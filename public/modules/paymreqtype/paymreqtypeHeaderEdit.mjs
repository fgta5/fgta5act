...
const CurrentSectionId = Context.Sections.paymreqtypeHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New PaymReq Type'
const TitleWhenView = 'View PaymReq Type'
const TitleWhenEdit = 'Edit PaymReq Type'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_new', 'paymreqtypeHeader-new')
const btn_del = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('paymreqtypeHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('paymreqtypeHeader-btn_recordstatus')
const btn_logs = document.getElementById('paymreqtypeHeader-btn_logs')
const btn_about = document.getElementById('paymreqtypeHeader-btn_about')
...
export async function init(self, args) {
	console.log('initializing paymreqtypeHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'paymreqtypeHeaderEdit_isEditDisabled'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.paymreqtypeHeaderList
...
async function btn_new_click(self, evt) {
	...
	const paymreqtypeHeaderList = self.Modules.paymreqtypeHeaderList
	const listsecid = paymreqtypeHeaderList.Section.Id
	...
	try {
		...
		if (fromListSection) {
			...
			self.Modules.paymreqtypeHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'paymreqtypeHeaderEdit_autofill'
	...
	try {
		...
		if (isNewData) {
			...
			self.Modules.paymreqtypeHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.paymreqtypeHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.paymreqtypeHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.paymreqtypeHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.paymreqtypeHeaderList.selectNextRow(self)
}
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About PaymReq Type'
