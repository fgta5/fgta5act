...
const CurrentSectionId = Context.Sections.paymreqtypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('paymreqtypeHeaderList-tbl')
const pnl_search = document.getElementById('paymreqtypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('paymreqtypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing paymreqtypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('paymreqtypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('paymreqtypeHeaderList-btn_gridload')
		...
		// paymreqtypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const paymreqtypeHeaderEdit = self.Modules.paymreqtypeHeaderEdit
		if (tbl.CurrentRow!=null) {
			paymreqtypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const paymreqtypeHeaderEdit = self.Modules.paymreqtypeHeaderEdit
	paymreqtypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await paymreqtypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, paymreqtypeHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const paymreqtypeHeaderEdit = self.Modules.paymreqtypeHeaderEdit
	paymreqtypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading paymreqtypeHeader data')
	console.log(params)
