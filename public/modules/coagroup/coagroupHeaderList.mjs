...
const CurrentSectionId = Context.Sections.coagroupHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('coagroupHeaderList-tbl')
const pnl_search = document.getElementById('coagroupHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('coagroupHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing coagroupHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('coagroupHeaderList-txt_searchtext')
		const btn_load = document.getElementById('coagroupHeaderList-btn_gridload')
		...
		// coagroupHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const coagroupHeaderEdit = self.Modules.coagroupHeaderEdit
		coagroupHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const coagroupHeaderEdit = self.Modules.coagroupHeaderEdit
	coagroupHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await coagroupHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, coagroupHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const coagroupHeaderEdit = self.Modules.coagroupHeaderEdit
	coagroupHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading coagroupHeader data')
	console.log(params)
