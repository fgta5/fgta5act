import Context from './itemmanage-context.mjs'
import * as Ext from './itemmanage-ext.mjs'
...
const CurrentSectionId = Context.Sections.itemmanageHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Item Manage'
const TitleWhenView = 'View Item Manage'
const TitleWhenEdit = 'Edit Item Manage'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_new', 'itemmanageHeader-new')
const btn_del = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('itemmanageHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('itemmanageHeader-btn_recordstatus')
const btn_logs = document.getElementById('itemmanageHeader-btn_logs')
const btn_about = document.getElementById('itemmanageHeader-btn_about')

const frm = new $fgta5.Form('itemmanageHeaderEdit-frm');
const obj_itemmanage_id = frm.Inputs['itemmanageHeaderEdit-obj_itemmanage_id']
const obj_itemmanage_name = frm.Inputs['itemmanageHeaderEdit-obj_itemmanage_name']	
...
export async function init(self, args) {
	console.log('initializing itemmanageHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'itemmanageHeaderEdit_isEditDisabled'
		...
		// export async function itemmanageHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'itemmanageHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.itemmanageHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function itemmanageHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'itemmanageHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function itemmanageHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'itemmanageHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const itemmanageHeaderList = self.Modules.itemmanageHeaderList
	const listsecid = itemmanageHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'itemmanageHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function itemmanageHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.itemmanageHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'itemmanageHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function itemmanageHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'itemmanageHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'itemmanageHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function itemmanageHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.itemmanageHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.itemmanageHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.itemmanageHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.itemmanageHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.itemmanageHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'itemmanageHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Item Manage'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 9 Dec 2025 15:05'
