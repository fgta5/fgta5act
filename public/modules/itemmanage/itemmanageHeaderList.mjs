...
const CurrentSectionId = Context.Sections.itemmanageHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('itemmanageHeaderList-tbl')
const pnl_search = document.getElementById('itemmanageHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('itemmanageHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing itemmanageHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('itemmanageHeaderList-txt_searchtext')
		const btn_load = document.getElementById('itemmanageHeaderList-btn_gridload')
		...
		// itemmanageHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const itemmanageHeaderEdit = self.Modules.itemmanageHeaderEdit
		if (tbl.CurrentRow!=null) {
			itemmanageHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const itemmanageHeaderEdit = self.Modules.itemmanageHeaderEdit
	itemmanageHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await itemmanageHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, itemmanageHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const itemmanageHeaderEdit = self.Modules.itemmanageHeaderEdit
	itemmanageHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading itemmanageHeader data')
	console.log(params)
