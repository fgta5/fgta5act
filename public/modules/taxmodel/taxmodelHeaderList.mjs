...
const CurrentSectionId = Context.Sections.taxmodelHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('taxmodelHeaderList-tbl')
const pnl_search = document.getElementById('taxmodelHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('taxmodelHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing taxmodelHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('taxmodelHeaderList-txt_searchtext')
		const btn_load = document.getElementById('taxmodelHeaderList-btn_gridload')
		...
		// taxmodelHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const taxmodelHeaderEdit = self.Modules.taxmodelHeaderEdit
		if (tbl.CurrentRow!=null) {
			taxmodelHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const taxmodelHeaderEdit = self.Modules.taxmodelHeaderEdit
	taxmodelHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await taxmodelHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, taxmodelHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const taxmodelHeaderEdit = self.Modules.taxmodelHeaderEdit
	taxmodelHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading taxmodelHeader data')
	console.log(params)
