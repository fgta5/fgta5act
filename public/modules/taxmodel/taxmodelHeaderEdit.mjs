import Context from './taxmodel-context.mjs'
import * as Ext from './taxmodel-ext.mjs'
...
const CurrentSectionId = Context.Sections.taxmodelHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Tax Model'
const TitleWhenView = 'View Tax Model'
const TitleWhenEdit = 'Edit Tax Model'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_new', 'taxmodelHeader-new')
const btn_del = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('taxmodelHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('taxmodelHeader-btn_recordstatus')
const btn_logs = document.getElementById('taxmodelHeader-btn_logs')
const btn_about = document.getElementById('taxmodelHeader-btn_about')

const frm = new $fgta5.Form('taxmodelHeaderEdit-frm');
const obj_taxmodel_id = frm.Inputs['taxmodelHeaderEdit-obj_taxmodel_id']
const obj_taxmodel_name = frm.Inputs['taxmodelHeaderEdit-obj_taxmodel_name']
const obj_taxmodel_formula = frm.Inputs['taxmodelHeaderEdit-obj_taxmodel_formula']
const obj_taxmodel_value = frm.Inputs['taxmodelHeaderEdit-obj_taxmodel_value']	
...
export async function init(self, args) {
	console.log('initializing taxmodelHeaderEdit ...')
	...
	// export async function taxmodelHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'taxmodelHeaderEdit_init'
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function taxmodelHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'taxmodelHeaderEdit_isEditDisabled'
		...
		// export async function taxmodelHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'taxmodelHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.taxmodelHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function taxmodelHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'taxmodelHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function taxmodelHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'taxmodelHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const taxmodelHeaderList = self.Modules.taxmodelHeaderList
	const listsecid = taxmodelHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'taxmodelHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function taxmodelHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.taxmodelHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'taxmodelHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function taxmodelHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'taxmodelHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'taxmodelHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function taxmodelHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.taxmodelHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.taxmodelHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.taxmodelHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.taxmodelHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.taxmodelHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'taxmodelHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Tax Model'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 15 Dec 2025 22:40'
