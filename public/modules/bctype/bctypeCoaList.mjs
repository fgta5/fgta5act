...
const CurrentSectionId = Context.Sections.bctypeCoaList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bctypeCoaList-tbl')

const btn_addrow = document.getElementById('bctypeCoaList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('bctypeCoaList-btn_delrow')
...
export async function init(self, args) {
	...
	// export function coaList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'coaList_addTableEvents'
	...
	// Extend list detil
	// export function bctypeCoaList_init(self) {}
	const fn_name = 'bctypeCoaList_init'
	const fn_bctypeCoaList_init = Extender[fn_name]
	if (typeof fn_bctypeCoaList_init === 'function') {
		fn_bctypeCoaList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.bctypeCoaList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function bctypeCoaList_openList(self, headerForm) {}
	const fn_name = 'bctypeCoaList_openList'
	const fn_bctypeCoaList_openList = Extender[fn_name]
	if (typeof fn_bctypeCoaList_openList === 'function') {
		fn_bctypeCoaList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			bctypeCoaEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const bctypeCoaEdit = self.Modules.bctypeCoaEdit
	bctypeCoaEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bctypeCoaEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, bctypeCoaEdit)
}
...
async function listRows(self, criteria, offset, limit, sort) {
	...
	// export function coaList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'coaList_dataLoad'
