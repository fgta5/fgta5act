...
const CurrentSectionId = Context.Sections.bctypeDeptList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bctypeDeptList-tbl')

const btn_addrow = document.getElementById('bctypeDeptList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('bctypeDeptList-btn_delrow')
...
export async function init(self, args) {
	...
	// export function deptList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'deptList_addTableEvents'
	...
	// Extend list detil
	// export function bctypeDeptList_init(self) {}
	const fn_name = 'bctypeDeptList_init'
	const fn_bctypeDeptList_init = Extender[fn_name]
	if (typeof fn_bctypeDeptList_init === 'function') {
		fn_bctypeDeptList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.bctypeDeptList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function bctypeDeptList_openList(self, headerForm) {}
	const fn_name = 'bctypeDeptList_openList'
	const fn_bctypeDeptList_openList = Extender[fn_name]
	if (typeof fn_bctypeDeptList_openList === 'function') {
		fn_bctypeDeptList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		const bctypeDeptEdit = self.Modules.bctypeDeptEdit
		if (tbl.CurrentRow!=null) {
			bctypeDeptEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const bctypeDeptEdit = self.Modules.bctypeDeptEdit
	bctypeDeptEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bctypeDeptEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, bctypeDeptEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/dept-list`
	const evt = { url, limit }

	// export function deptList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'deptList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/dept-delete-rows`
	try {
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export function deptList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'deptList_tableDataLoaded'
