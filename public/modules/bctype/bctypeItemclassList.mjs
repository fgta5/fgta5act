...
const CurrentSectionId = Context.Sections.bctypeItemclassList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bctypeItemclassList-tbl')

const btn_addrow = document.getElementById('bctypeItemclassList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('bctypeItemclassList-btn_delrow')
...
export async function init(self, args) {
	...
	CurrentSection.addEventListener($fgta5.Section.EVT_BACKBUTTONCLICK, async (evt)=>{
		const sectionId =  Context.Sections.bctypeHeaderEdit
	...
	// export function itemclassList_addTableEvents(self, tbl) {}
	const fn_addTableEvents_name = 'itemclassList_addTableEvents'
	...
	// Extend list detil
	// export function bctypeItemclassList_init(self) {}
	const fn_name = 'bctypeItemclassList_init'
	const fn_bctypeItemclassList_init = Extender[fn_name]
	if (typeof fn_bctypeItemclassList_init === 'function') {
		fn_bctypeItemclassList_init(self)
	}
...
export async function openList(self, params) {
	...
	// bisa di set pada Extender.bctypeItemclassList_openList :  bisa menggunakan template untuk di embed ke header pada detil list
	// export function bctypeItemclassList_openList(self, headerForm) {}
	const fn_name = 'bctypeItemclassList_openList'
	const fn_bctypeItemclassList_openList = Extender[fn_name]
	if (typeof fn_bctypeItemclassList_openList === 'function') {
		fn_bctypeItemclassList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		const bctypeItemclassEdit = self.Modules.bctypeItemclassEdit
		if (tbl.CurrentRow!=null) {
			bctypeItemclassEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const bctypeItemclassEdit = self.Modules.bctypeItemclassEdit
	bctypeItemclassEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bctypeItemclassEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, bctypeItemclassEdit)
}


async function listRows(self, criteria, offset, limit, sort) {
	const url = `/${Context.moduleName}/itemclass-list`
	const evt = { url, limit }

	// export function itemclassList_dataLoad(self, criteria, sort, evt) {}
	const fn_dataLoad_name = 'itemclassList_dataLoad'
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/itemclass-delete-rows`
	try {
...
async function tbl_loadData(self, params={}) {
	...
	try {
		...
		// export function itemclassList_tableDataLoaded(self, tbl, result) {}
		const fn_name = 'itemclassList_tableDataLoaded'
