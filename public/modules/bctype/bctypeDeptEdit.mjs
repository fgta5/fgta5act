...
const Extender = Ext.extenderDept ?? Ext


const CurrentState = {}
const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.bctypeDeptEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source

const TitleWhenNew = 'New Departemen'
const TitleWhenView = 'View Departemen'
const TitleWhenEdit = 'Edit Departemen'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('bctypeDeptEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('bctypeDeptEdit-btn_save')
const btn_new = new $fgta5.ActionButton('bctypeDeptEdit-btn_new', 'bctypeDept-addrow')
const btn_del = new $fgta5.ActionButton('bctypeDeptEdit-btn_delete', 'bctypeDept-delrow')
const btn_reset = new $fgta5.ActionButton('bctypeDeptEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('bctypeDeptEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('bctypeDeptEdit-btn_next')

const btn_recordstatus = document.getElementById('bctypeDept-btn_recordstatus')
const btn_logs = document.getElementById('bctypeDept-btn_logs')

const frm = new $fgta5.Form('bctypeDeptEdit-frm');
const obj_bctypedept_id = frm.Inputs['bctypeDeptEdit-obj_bctypedept_id']
const obj_dept_id = frm.Inputs['bctypeDeptEdit-obj_dept_id']
const obj_bctype_id = frm.Inputs['bctypeDeptEdit-obj_bctype_id']	
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'bctypeDeptEdit_isEditDisabled'
		...
		// export function bctypeDeptEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'bctypeDeptEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function bctypeDeptEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'bctypeDeptEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function bctypeDeptEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'bctypeDeptEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/dept-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/dept-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/dept-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/dept-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.bctypeDeptList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function bctypeDeptEdit_formUnlocked(self, frm) {}
	const fn_name = 'bctypeDeptEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const bctypeDeptList = self.Modules.bctypeDeptList
	const listsecid = bctypeDeptList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(bctypeDeptList.Section)
	...
	try {
		...
		// export async function bctypeDeptEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'bctypeDeptEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.bctypeDeptList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function bctypeDeptEdit_autofill(self, frm) {}
	const fn_autofill_name = 'bctypeDeptEdit_autofill'
	...
	// Extender Saving
	// export async function bctypeDeptEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'bctypeDeptEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		// export async function bctypeDeptEdit_dataSaved(self, data, frm) {}
		const fn_datasaved_name = 'bctypeDeptEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.bctypeDeptList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.bctypeDeptList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.bctypeDeptList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.bctypeDeptList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.bctypeDeptList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender bctypeDeptEdit_addRecordInfo
			// export async function bctypeDeptEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'bctypeDeptEdit_addRecordInfo'
