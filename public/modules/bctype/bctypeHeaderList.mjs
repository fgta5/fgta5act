...
const CurrentSectionId = Context.Sections.bctypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bctypeHeaderList-tbl')
const pnl_search = document.getElementById('bctypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('bctypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing bctypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('bctypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('bctypeHeaderList-btn_gridload')
		...
		// bctypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
async function openRow(self, tr) {
	...
	const bctypeHeaderEdit = self.Modules.bctypeHeaderEdit
	bctypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bctypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, bctypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const bctypeHeaderEdit = self.Modules.bctypeHeaderEdit
	bctypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading bctypeHeader data')
	console.log(params)
