...
const Extender = Ext.extenderItemclass ?? Ext


const CurrentState = {}
const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.bctypeItemclassEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source

const TitleWhenNew = 'New ItemClass'
const TitleWhenView = 'View ItemClass'
const TitleWhenEdit = 'Edit ItemClass'
const EditModeText = 'Edit'
const LockModeText = 'Lock'



const btn_edit = new $fgta5.ActionButton('bctypeItemclassEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('bctypeItemclassEdit-btn_save')
const btn_new = new $fgta5.ActionButton('bctypeItemclassEdit-btn_new', 'bctypeItemclass-addrow')
const btn_del = new $fgta5.ActionButton('bctypeItemclassEdit-btn_delete', 'bctypeItemclass-delrow')
const btn_reset = new $fgta5.ActionButton('bctypeItemclassEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('bctypeItemclassEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('bctypeItemclassEdit-btn_next')

const btn_recordstatus = document.getElementById('bctypeItemclass-btn_recordstatus')
const btn_logs = document.getElementById('bctypeItemclass-btn_logs')

const frm = new $fgta5.Form('bctypeItemclassEdit-frm');
const obj_bctypeitemclass_id = frm.Inputs['bctypeItemclassEdit-obj_bctypeitemclass_id']
const obj_itemclass_id = frm.Inputs['bctypeItemclassEdit-obj_itemclass_id']
const obj_bctype_id = frm.Inputs['bctypeItemclassEdit-obj_bctype_id']	
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'bctypeItemclassEdit_isEditDisabled'
		...
		// export function bctypeItemclassEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'bctypeItemclassEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function bctypeItemclassEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'bctypeItemclassEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function bctypeItemclassEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'bctypeItemclassEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/itemclass-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/itemclass-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/itemclass-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/itemclass-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.bctypeItemclassList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function bctypeItemclassEdit_formUnlocked(self, frm) {}
	const fn_name = 'bctypeItemclassEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const bctypeItemclassList = self.Modules.bctypeItemclassList
	const listsecid = bctypeItemclassList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(bctypeItemclassList.Section)
	...
	try {
		...
		// export async function bctypeItemclassEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'bctypeItemclassEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.bctypeItemclassList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function bctypeItemclassEdit_autofill(self, frm) {}
	const fn_autofill_name = 'bctypeItemclassEdit_autofill'
	...
	// Extender Saving
	// export async function bctypeItemclassEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'bctypeItemclassEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		// export async function bctypeItemclassEdit_dataSaved(self, data, frm) {}
		const fn_datasaved_name = 'bctypeItemclassEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.bctypeItemclassList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.bctypeItemclassList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.bctypeItemclassList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.bctypeItemclassList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.bctypeItemclassList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender bctypeItemclassEdit_addRecordInfo
			// export async function bctypeItemclassEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'bctypeItemclassEdit_addRecordInfo'
