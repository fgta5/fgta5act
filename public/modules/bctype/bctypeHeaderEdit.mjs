...
const CurrentSectionId = Context.Sections.bctypeHeaderEdit
...
const btn_edit = new $fgta5.ActionButton('bctypeHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('bctypeHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('bctypeHeaderEdit-btn_new', 'bctypeHeader-new')
const btn_del = new $fgta5.ActionButton('bctypeHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('bctypeHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('bctypeHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('bctypeHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('bctypeHeader-btn_recordstatus')
const btn_logs = document.getElementById('bctypeHeader-btn_logs')
const btn_about = document.getElementById('bctypeHeader-btn_about')
...
export async function init(self, args) {
	console.log('initializing bctypeHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_formopened_name = 'bctypeHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.bctypeHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function bctypeHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'bctypeHeaderEdit_formLocked'
	...
	self.Modules.bctypeCoaList.headerLocked(self)
	self.Modules.bctypeCoaEdit.headerLocked(self)
	
	// trigger lock event di itemclass
	self.Modules.bctypeItemclassList.headerLocked(self)
	self.Modules.bctypeItemclassEdit.headerLocked(self)
	
	// trigger lock event di dept
	self.Modules.bctypeDeptList.headerLocked(self)
	self.Modules.bctypeDeptEdit.headerLocked(self)
...
async function  frm_unlocked(self, evt) {
	...
	// export function bctypeHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'bctypeHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
	...
	self.Modules.bctypeCoaList.headerUnlocked(self)
	self.Modules.bctypeCoaEdit.headerUnlocked(self)	
	
	// trigger unlock event di itemclass
	self.Modules.bctypeItemclassList.headerUnlocked(self)
	self.Modules.bctypeItemclassEdit.headerUnlocked(self)	
	
	// trigger unlock event di dept
	self.Modules.bctypeDeptList.headerUnlocked(self)
	self.Modules.bctypeDeptEdit.headerUnlocked(self)	
...
async function btn_new_click(self, evt) {
	...
	const bctypeHeaderList = self.Modules.bctypeHeaderList
	const listsecid = bctypeHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'bctypeHeaderEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.bctypeHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'bctypeHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function bctypeHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'bctypeHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'bctypeHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function bctypeHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.bctypeHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.bctypeHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.bctypeHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.bctypeHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.bctypeHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'bctypeHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		if ( document.getElementById('fAbout-section-fdescr') == null) {
			...
			divDescr.innerHTML = 'tipe bussiness case'
