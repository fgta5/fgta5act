...
const TitleWhenNew = 'New Item Class'
const TitleWhenView = 'View Item Class'
const TitleWhenEdit = 'Edit Item Class'
...
const obj_itemclasstype_id = frm.Inputs['itemclassHeaderEdit-obj_itemclasstype_id']
const obj_dept_id = frm.Inputs['itemclassHeaderEdit-obj_dept_id']
const obj_itemclass_name = frm.Inputs['itemclassHeaderEdit-obj_itemclass_name']
const obj_itemmanage_id = frm.Inputs['itemclassHeaderEdit-obj_itemmanage_id']
const obj_bs_coa_id = frm.Inputs['itemclassHeaderEdit-obj_bs_coa_id']
const obj_depre_coa_id = frm.Inputs['itemclassHeaderEdit-obj_depre_coa_id']
const obj_depremodel_id = frm.Inputs['itemclassHeaderEdit-obj_depremodel_id']
const obj_directexpensecond = frm.Inputs['itemclassHeaderEdit-obj_directexpensecond']
const obj_bellowvalue = frm.Inputs['itemclassHeaderEdit-obj_bellowvalue']
const obj_expense_coa_id = frm.Inputs['itemclassHeaderEdit-obj_expense_coa_id']	
...
export async function init(self, args) {
	...
	// Combobox: obj_itemclasstype_id
	obj_itemclasstype_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_itemclasstype_id_selected'
		...
		if (typeof fn_selected === 'function') {
			...
			fn_selected(self, obj_itemclasstype_id, frm, evt)
		} else {	
			console.warn('Extender.obj_itemclasstype_id_selected is not implemented')
		}		
	})
	
	obj_itemclasstype_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_itemclasstype_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_itemclasstype_id_selecting(self, obj_itemclasstype_id, frm, evt) {}
			fn_selecting(self, obj_itemclasstype_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'itemclasstype/header-list'
			const sort = {}
			const criteria = {
			...
			const fn_selecting_criteria_name = 'obj_itemclasstype_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_itemclasstype_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.itemclasstype_id, row.itemclasstype_name, row)
				}
	...
	// Combobox: obj_bs_coa_id
	obj_bs_coa_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_bs_coa_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_bs_coa_id_selecting(self, obj_bs_coa_id, frm, evt) {}
			fn_selecting(self, obj_bs_coa_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			...
			// buat function di extender:
			// export function obj_bs_coa_id_selecting_criteria(self, obj_bs_coa_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_bs_coa_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_bs_coa_id, frm, criteria, sort, evt)
			}
	...
	// Combobox: obj_depre_coa_id
	obj_depre_coa_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_depre_coa_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_depre_coa_id_selecting(self, obj_depre_coa_id, frm, evt) {}
			fn_selecting(self, obj_depre_coa_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			...
			// buat function di extender:
			// export function obj_depre_coa_id_selecting_criteria(self, obj_depre_coa_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_depre_coa_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_depre_coa_id, frm, criteria, sort, evt)
			}
	...
	// Combobox: obj_depremodel_id
	obj_depremodel_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_depremodel_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_depremodel_id_selecting(self, obj_depremodel_id, frm, evt) {}
			fn_selecting(self, obj_depremodel_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'depremodel/header-list'
			const sort = {}
			const criteria = {
				searchtext: searchtext,
			}
			...
			// buat function di extender:
			// export function obj_depremodel_id_selecting_criteria(self, obj_depremodel_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_depremodel_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_depremodel_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.depremodel_id, row.depremodel_name, row)
				}
	...
	// Combobox: obj_expense_coa_id
	obj_expense_coa_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_expense_coa_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_expense_coa_id_selecting(self, obj_expense_coa_id, frm, evt) {}
			fn_selecting(self, obj_expense_coa_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			...
			// buat function di extender:
			// export function obj_expense_coa_id_selecting_criteria(self, obj_expense_coa_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_expense_coa_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_expense_coa_id, frm, criteria, sort, evt)
			}
...
export async function openSelectedData(self, params) {
	...
	try {
		obj_itemclasstype_id.clear()
		obj_dept_id.clear()
		obj_bs_coa_id.clear()
		obj_depre_coa_id.clear()
		obj_depremodel_id.clear()
		obj_expense_coa_id.clear()
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Item Class'
		...
		if ( document.getElementById('fAbout-section-fdescr') == null) {
			...
			divDescr.innerHTML = 'klasifikasi barang (berwujut, tak berwujud) / jasa'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 14 Dec 2025 02:11'
