import Context from './depremodel-context.mjs'
import * as Ext from './depremodel-ext.mjs'
...
const CurrentSectionId = Context.Sections.depremodelHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Depresiasi'
const TitleWhenView = 'View Depresiasi'
const TitleWhenEdit = 'Edit Depresiasi'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('depremodelHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('depremodelHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('depremodelHeaderEdit-btn_new', 'depremodelHeader-new')
const btn_del = new $fgta5.ActionButton('depremodelHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('depremodelHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('depremodelHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('depremodelHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('depremodelHeader-btn_recordstatus')
const btn_logs = document.getElementById('depremodelHeader-btn_logs')
const btn_about = document.getElementById('depremodelHeader-btn_about')

const frm = new $fgta5.Form('depremodelHeaderEdit-frm');
const obj_depremodel_id = frm.Inputs['depremodelHeaderEdit-obj_depremodel_id']
const obj_depremodel_name = frm.Inputs['depremodelHeaderEdit-obj_depremodel_name']
const obj_formulaname = frm.Inputs['depremodelHeaderEdit-obj_formulaname']	
...
export async function init(self, args) {
	console.log('initializing depremodelHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'depremodelHeaderEdit_isEditDisabled'
		...
		// export async function depremodelHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'depremodelHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.depremodelHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function depremodelHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'depremodelHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function depremodelHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'depremodelHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const depremodelHeaderList = self.Modules.depremodelHeaderList
	const listsecid = depremodelHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'depremodelHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function depremodelHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.depremodelHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'depremodelHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function depremodelHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'depremodelHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'depremodelHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function depremodelHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.depremodelHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.depremodelHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.depremodelHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.depremodelHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.depremodelHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'depremodelHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Depresiasi'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 7 Dec 2025 20:46'
