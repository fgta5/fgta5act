...
const CurrentSectionId = Context.Sections.depremodelHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('depremodelHeaderList-tbl')
const pnl_search = document.getElementById('depremodelHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('depremodelHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing depremodelHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('depremodelHeaderList-txt_searchtext')
		const btn_load = document.getElementById('depremodelHeaderList-btn_gridload')
		...
		// depremodelHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const depremodelHeaderEdit = self.Modules.depremodelHeaderEdit
		if (tbl.CurrentRow!=null) {
			depremodelHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const depremodelHeaderEdit = self.Modules.depremodelHeaderEdit
	depremodelHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await depremodelHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, depremodelHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const depremodelHeaderEdit = self.Modules.depremodelHeaderEdit
	depremodelHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading depremodelHeader data')
	console.log(params)
