...
const CurrentSectionId = Context.Sections.jurnaltypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('jurnaltypeHeaderList-tbl')
const pnl_search = document.getElementById('jurnaltypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('jurnaltypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing jurnaltypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('jurnaltypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('jurnaltypeHeaderList-btn_gridload')
		...
		// jurnaltypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			jurnaltypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const jurnaltypeHeaderEdit = self.Modules.jurnaltypeHeaderEdit
	jurnaltypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await jurnaltypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, jurnaltypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const jurnaltypeHeaderEdit = self.Modules.jurnaltypeHeaderEdit
	jurnaltypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading jurnaltypeHeader data')
	console.log(params)
