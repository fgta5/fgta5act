...
const btn_edit = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_save')
const btn_new = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_new', 'jurnaltypeUser-addrow')
const btn_del = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_delete', 'jurnaltypeUser-delrow')
const btn_reset = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('jurnaltypeUserEdit-btn_next')

const btn_recordstatus = document.getElementById('jurnaltypeUser-btn_recordstatus')
const btn_logs = document.getElementById('jurnaltypeUser-btn_logs')

const frm = new $fgta5.Form('jurnaltypeUserEdit-frm');
const obj_jurnaltypeuser_id = frm.Inputs['jurnaltypeUserEdit-obj_jurnaltypeuser_id']
const obj_user_id = frm.Inputs['jurnaltypeUserEdit-obj_user_id']
const obj_isallowposting = frm.Inputs['jurnaltypeUserEdit-obj_isallowposting']
const obj_isallowunposting = frm.Inputs['jurnaltypeUserEdit-obj_isallowunposting']
const obj_jurnaltype_id = frm.Inputs['jurnaltypeUserEdit-obj_jurnaltype_id']	
...
export async function init(self, args) {
	...
	obj_user_id.addEventListener('selecting', async (evt)=>{
		...
		if (typeof fn_selecting === 'function') {
			...
			const url = `${Context.appsUrls.core.url}/user/header-list`
			const sort = {}
			const criteria = {
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		if (!CurrentState.headerFormLocked) {
			const fn_iseditdisabled_name = 'jurnaltypeUserEdit_isEditDisabled'
		...
		// export function jurnaltypeUserEdit_formOpened(self, frm, CurrentState) {}
		const fn_formopened_name = 'jurnaltypeUserEdit_formOpened'
...
export function headerLocked(self) {
	...
	// export function jurnaltypeUserEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'jurnaltypeUserEdit_formLocked'
...
export function headerUnlocked(self) {
	...
	// export function jurnaltypeUserEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'jurnaltypeUserEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}	
...
async function openData(self, id) {
	const url = `/${Context.moduleName}/user-open`
	try {
...
async function createData(self, data, formData) {
	const url = `/${Context.moduleName}/user-create`
	try {
...
async function updateData(self, data, formData) {
	const url = `/${Context.moduleName}/user-update`
	try {
		const result = await Module.apiCall(url, { data, source: Source }, formData) 
		return result 
	} catch (err) {
...
async function deleteData(self, id) {
	const url = `/${Context.moduleName}/user-delete`
	try {
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.jurnaltypeUserList
...
async function  frm_unlocked(self, evt) {
	...
	// Extender untuk event Unlocked
	// export function jurnaltypeUserEdit_formUnlocked(self, frm) {}
	const fn_name = 'jurnaltypeUserEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm)
...
async function btn_new_click(self, evt) {
	...
	const jurnaltypeUserList = self.Modules.jurnaltypeUserList
	const listsecid = jurnaltypeUserList.Section.Id
	const fromListSection = sourceSection===listsecid

	if (fromListSection) {
		console.log('tambahkan row baru')
		CurrentSection.setSectionReturn(jurnaltypeUserList.Section)
	...
	try {
		...
		// export async function jurnaltypeUserEdit_newData(self, datainit, frm, CurrentState) {}
		const fn_newdata_name = 'jurnaltypeUserEdit_newData'
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	// export async function jurnaltypeUserEdit_autofill(self, frm) {}
	const fn_autofill_name = 'jurnaltypeUserEdit_autofill'
	...
	// Extender Saving
	// export async function jurnaltypeUserEdit_dataSaving(self, dataToSave, frm) {}
	const fn_datasaving_name = 'jurnaltypeUserEdit_dataSaving'
	...
	try {
		...
		if (isNewData) {
			...
			self.Modules.jurnaltypeUserList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.jurnaltypeUserList.updateCurrentRow(self, data)
		}
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.jurnaltypeUserList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.jurnaltypeUserList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			// melalui extender jurnaltypeUserEdit_addRecordInfo
			// export async function jurnaltypeUserEdit_addRecordInfo(self,  data) {}
			const fn_addrecordinfo_name = 'jurnaltypeUserEdit_addRecordInfo'
