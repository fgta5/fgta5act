...
const CurrentSectionId = Context.Sections.paymreqTermList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('paymreqTermList-tbl')

const btn_addrow = document.getElementById('paymreqTermList-btn_addrow') // tidak perlu pakai action, karna action didefine di edit
const btn_delrow = new $fgta5.ActionButton('paymreqTermList-btn_delrow')
...
export async function openList(self, params) {
	...
	if (typeof fn_paymreqTermList_openList === 'function') {
		fn_paymreqTermList_openList(self, headerForm)
	}
...
export function keyboardAction(self, actionName) {
	if (actionName=='up') {
		...
		const paymreqTermEdit = self.Modules.paymreqTermEdit
		if (tbl.CurrentRow!=null) {
			paymreqTermEdit.Section.show({}, (evt)=>{
...
async function openRow(self, tr) {
	...
	const paymreqTermEdit = self.Modules.paymreqTermEdit
	paymreqTermEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await paymreqTermEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, paymreqTermEdit)
}
...
async function deleteRows(self, data) {
	const url = `/${Context.moduleName}/term-delete-rows`
	try {
