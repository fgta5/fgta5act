...
const CurrentSectionId = Context.Sections.paymreqHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('paymreqHeaderList-tbl')
const pnl_search = document.getElementById('paymreqHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('paymreqHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing paymreqHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('paymreqHeaderList-txt_searchtext')
		const btn_load = document.getElementById('paymreqHeaderList-btn_gridload')
		...
		// paymreqHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		if (tbl.CurrentRow!=null) {
			paymreqHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const paymreqHeaderEdit = self.Modules.paymreqHeaderEdit
	paymreqHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await paymreqHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, paymreqHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const paymreqHeaderEdit = self.Modules.paymreqHeaderEdit
	paymreqHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading paymreqHeader data')
	console.log(params)
