import Context from './agingtype-context.mjs'
import * as Extender from './agingtype-ext.mjs'
import * as pageHelper from '/public/libs/webmodule/pagehelper.mjs'

const CurrentState = {}
const Crsl =  Context.Crsl
const CurrentSectionId = Context.Sections.agingtypeHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Aging Type'
const TitleWhenView = 'View Aging Type'
const TitleWhenEdit = 'Edit Aging Type'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_new', 'agingtypeHeader-new')
const btn_del = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('agingtypeHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('agingtypeHeader-btn_recordstatus')
const btn_logs = document.getElementById('agingtypeHeader-btn_logs')
const btn_about = document.getElementById('agingtypeHeader-btn_about')

const frm = new $fgta5.Form('agingtypeHeaderEdit-frm');
const obj_agingtype_id = frm.Inputs['agingtypeHeaderEdit-obj_agingtype_id']
const obj_agingtype_name = frm.Inputs['agingtypeHeaderEdit-obj_agingtype_name']
const obj_agingtype_Descr = frm.Inputs['agingtypeHeaderEdit-obj_agingtype_Descr']	
...
export async function init(self, args) {
	console.log('initializing agingtypeHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'agingtypeHeaderEdit_isEditDisabled'
		...
		const fn_formopened_name = 'agingtypeHeaderEdit_formOpened'
		const fn_formopened = Extender[fn_formopened_name]
		if (typeof fn_formopened === 'function') {
			// export async function agingtypeHeaderEdit_formOpened(self, frm, CurrentState)
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.agingtypeHeaderList
...
async function  frm_locked(self, evt) {
	...
	const fn_name = 'agingtypeHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	const fn_name = 'agingtypeHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const agingtypeHeaderList = self.Modules.agingtypeHeaderList
	const listsecid = agingtypeHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'agingtypeHeaderEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.agingtypeHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'agingtypeHeaderEdit_autofill'
	...
	// Extender Saving
	const fn_datasaving_name = 'agingtypeHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'agingtypeHeaderEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.agingtypeHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.agingtypeHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.agingtypeHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.agingtypeHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.agingtypeHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'agingtypeHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Aging Type'
		...
		if ( document.getElementById('fAbout-section-fdescr') == null) {
			...
			divDescr.innerHTML = 'Definisi Tipe Aging'
