...
const CurrentSectionId = Context.Sections.agingtypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('agingtypeHeaderList-tbl')
const pnl_search = document.getElementById('agingtypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('agingtypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing agingtypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('agingtypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('agingtypeHeaderList-btn_gridload')
		...
		// agingtypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const agingtypeHeaderEdit = self.Modules.agingtypeHeaderEdit
		agingtypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const agingtypeHeaderEdit = self.Modules.agingtypeHeaderEdit
	agingtypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await agingtypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, agingtypeHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const agingtypeHeaderEdit = self.Modules.agingtypeHeaderEdit
	agingtypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading agingtypeHeader data')
	console.log(params)
