...
const CurrentSectionId = Context.Sections.bccycleHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bccycleHeaderList-tbl')
const pnl_search = document.getElementById('bccycleHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('bccycleHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing bccycleHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('bccycleHeaderList-txt_searchtext')
		const btn_load = document.getElementById('bccycleHeaderList-btn_gridload')
		...
		// bccycleHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const bccycleHeaderEdit = self.Modules.bccycleHeaderEdit
		if (tbl.CurrentRow!=null) {
			bccycleHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const bccycleHeaderEdit = self.Modules.bccycleHeaderEdit
	bccycleHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bccycleHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, bccycleHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const bccycleHeaderEdit = self.Modules.bccycleHeaderEdit
	bccycleHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading bccycleHeader data')
	console.log(params)
