import Context from './bccycle-context.mjs'
import * as Ext from './bccycle-ext.mjs'
...
const CurrentSectionId = Context.Sections.bccycleHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Business Cycle'
const TitleWhenView = 'View Business Cycle'
const TitleWhenEdit = 'Edit Business Cycle'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('bccycleHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('bccycleHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('bccycleHeaderEdit-btn_new', 'bccycleHeader-new')
const btn_del = new $fgta5.ActionButton('bccycleHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('bccycleHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('bccycleHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('bccycleHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('bccycleHeader-btn_recordstatus')
const btn_logs = document.getElementById('bccycleHeader-btn_logs')
const btn_about = document.getElementById('bccycleHeader-btn_about')

const frm = new $fgta5.Form('bccycleHeaderEdit-frm');
const obj_bccycle_id = frm.Inputs['bccycleHeaderEdit-obj_bccycle_id']
const obj_bccycle_name = frm.Inputs['bccycleHeaderEdit-obj_bccycle_name']
const obj_isuseclass = frm.Inputs['bccycleHeaderEdit-obj_isuseclass']
const obj_isusecurr = frm.Inputs['bccycleHeaderEdit-obj_isusecurr']
const obj_isuseapproval = frm.Inputs['bccycleHeaderEdit-obj_isuseapproval']
const obj_isusepaymreq = frm.Inputs['bccycleHeaderEdit-obj_isusepaymreq']	
...
export async function init(self, args) {
	console.log('initializing bccycleHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'bccycleHeaderEdit_isEditDisabled'
		...
		// export async function bccycleHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'bccycleHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.bccycleHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function bccycleHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'bccycleHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function bccycleHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'bccycleHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const bccycleHeaderList = self.Modules.bccycleHeaderList
	const listsecid = bccycleHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'bccycleHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function bccycleHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.bccycleHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'bccycleHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function bccycleHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'bccycleHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'bccycleHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function bccycleHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.bccycleHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.bccycleHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.bccycleHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.bccycleHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.bccycleHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'bccycleHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Business Cycle'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 10 Dec 2025 12:14'
