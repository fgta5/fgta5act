...
const CurrentSectionId = Context.Sections.bcHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Bussiness Case'
const TitleWhenView = 'View Bussiness Case'
const TitleWhenEdit = 'Edit Bussiness Case'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('bcHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('bcHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('bcHeaderEdit-btn_new', 'bcHeader-new')
const btn_del = new $fgta5.ActionButton('bcHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('bcHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('bcHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('bcHeaderEdit-btn_next')

const btn_actionCommit = new $fgta5.ActionButton('bcHeaderEdit-btn_actionCommit')

const btn_recordstatus = document.getElementById('bcHeader-btn_recordstatus')
const btn_logs = document.getElementById('bcHeader-btn_logs')
const btn_about = document.getElementById('bcHeader-btn_about')

const frm = new $fgta5.Form('bcHeaderEdit-frm');
const obj_bc_id = frm.Inputs['bcHeaderEdit-obj_bc_id']
const obj_bc_doc = frm.Inputs['bcHeaderEdit-obj_bc_doc']
const obj_isapproved = frm.Inputs['bcHeaderEdit-obj_isapproved']
const obj_bc_date = frm.Inputs['bcHeaderEdit-obj_bc_date']
const obj_dept_id = frm.Inputs['bcHeaderEdit-obj_dept_id']
const obj_bctype_id = frm.Inputs['bcHeaderEdit-obj_bctype_id']
const obj_bc_title = frm.Inputs['bcHeaderEdit-obj_bc_title']
const obj_bc_descr = frm.Inputs['bcHeaderEdit-obj_bc_descr']
const obj_curr_id = frm.Inputs['bcHeaderEdit-obj_curr_id']
const obj_coa_id = frm.Inputs['bcHeaderEdit-obj_coa_id']
const obj_bc_value = frm.Inputs['bcHeaderEdit-obj_bc_value']
const obj_paymreqtype_id = frm.Inputs['bcHeaderEdit-obj_paymreqtype_id']
const obj_approvalmodel_id = frm.Inputs['bcHeaderEdit-obj_approvalmodel_id']	
...
export async function init(self, args) {
	console.log('initializing bcHeaderEdit ...')
	...
	// export async function bcHeaderEdit_init(self, CurrentState)
	const fn_init_name = 'bcHeaderEdit_init'
	...
	obj_dept_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_dept_id_selected'
		...
		if (typeof fn_selected === 'function') {
			...
			fn_selected(self, obj_dept_id, frm, evt)
		} else {	
			console.warn('Extender.obj_dept_id_selected is not implemented')
		}		
	})
	...
	// Combobox: obj_bctype_id
	obj_bctype_id.addEventListener('selected', (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selected_name = 'obj_bctype_id_selected'
		const fn_selected = Extender[fn_selected_name]
		if (typeof fn_selected === 'function') {
			// create function di Extender:
			// export async function obj_bctype_id_selected(self, obj_bctype_id, frm, evt) {}
			fn_selected(self, obj_bctype_id, frm, evt)
		} else {	
			console.warn('Extender.obj_bctype_id_selected is not implemented')
		}		
	})
	
	obj_bctype_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_bctype_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_bctype_id_selecting(self, obj_bctype_id, frm, evt) {}
			fn_selecting(self, obj_bctype_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'bctype-filtered/list-by-dept'
			const sort = {}
			const criteria = {
				searchtext: searchtext,
			}
			...
			const fn_selecting_criteria_name = 'obj_bctype_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_bctype_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.bctype_id, row.bctype_name, row)
				}
	...
	obj_curr_id.addEventListener('selecting', async (evt)=>{
		...
		if (typeof fn_selecting === 'function') {
			...
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'bctype-filtered/list-curr'
			const sort = {}
			const criteria = {
				searchtext: searchtext,
			}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		// export async function bcHeaderEdit_isEditDisabled(self, data)
		const fn_iseditdisabled_name = 'bcHeaderEdit_isEditDisabled'
		...
		const fn_formopened_name = 'bcHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.bcHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function bcHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'bcHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function bcHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'bcHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const bcHeaderList = self.Modules.bcHeaderList
	const listsecid = bcHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'bcHeaderEdit_newData'
		...
		if (fromListSection) {
			...
			self.Modules.bcHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'bcHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function bcHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'bcHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'bcHeaderEdit_dataSaved'
		...
		if (isNewData) {
			...
			self.Modules.bcHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.bcHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.bcHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.bcHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.bcHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'bcHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Bussiness Case'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 16 Dec 2025 14:18'
