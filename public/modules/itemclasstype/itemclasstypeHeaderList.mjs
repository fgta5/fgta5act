...
const CurrentSectionId = Context.Sections.itemclasstypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('itemclasstypeHeaderList-tbl')
const pnl_search = document.getElementById('itemclasstypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('itemclasstypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing itemclasstypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('itemclasstypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('itemclasstypeHeaderList-btn_gridload')
		...
		// itemclasstypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const itemclasstypeHeaderEdit = self.Modules.itemclasstypeHeaderEdit
		if (tbl.CurrentRow!=null) {
			itemclasstypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const itemclasstypeHeaderEdit = self.Modules.itemclasstypeHeaderEdit
	itemclasstypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await itemclasstypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, itemclasstypeHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const itemclasstypeHeaderEdit = self.Modules.itemclasstypeHeaderEdit
	itemclasstypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading itemclasstypeHeader data')
	console.log(params)
