import Context from './itemclasstype-context.mjs'
import * as Ext from './itemclasstype-ext.mjs'
...
const CurrentSectionId = Context.Sections.itemclasstypeHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New Item Class Type'
const TitleWhenView = 'View Item Class Type'
const TitleWhenEdit = 'Edit Item Class Type'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_new', 'itemclasstypeHeader-new')
const btn_del = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('itemclasstypeHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('itemclasstypeHeader-btn_recordstatus')
const btn_logs = document.getElementById('itemclasstypeHeader-btn_logs')
const btn_about = document.getElementById('itemclasstypeHeader-btn_about')

const frm = new $fgta5.Form('itemclasstypeHeaderEdit-frm');
const obj_itemclasstype_id = frm.Inputs['itemclasstypeHeaderEdit-obj_itemclasstype_id']
const obj_itemclasstype_name = frm.Inputs['itemclasstypeHeaderEdit-obj_itemclasstype_name']
const obj_itemmanage_id = frm.Inputs['itemclasstypeHeaderEdit-obj_itemmanage_id']
const obj_itemclasstype_descr = frm.Inputs['itemclasstypeHeaderEdit-obj_itemclasstype_descr']
const obj_ishasbscoa = frm.Inputs['itemclasstypeHeaderEdit-obj_ishasbscoa']
const obj_ishasdeprecoa = frm.Inputs['itemclasstypeHeaderEdit-obj_ishasdeprecoa']
const obj_ishasdepremethod = frm.Inputs['itemclasstypeHeaderEdit-obj_ishasdepremethod']
const obj_ishasdeprecond = frm.Inputs['itemclasstypeHeaderEdit-obj_ishasdeprecond']
const obj_ishasexpensecoa = frm.Inputs['itemclasstypeHeaderEdit-obj_ishasexpensecoa']	
...
export async function init(self, args) {
	console.log('initializing itemclasstypeHeaderEdit ...')
	...
	// Combobox: obj_itemmanage_id
	obj_itemmanage_id.addEventListener('selecting', async (evt)=>{
		
		evt.detail.CurrentState = CurrentState
		
		const fn_selecting_name = 'obj_itemmanage_id_selecting'
		const fn_selecting = Extender[fn_selecting_name]
		if (typeof fn_selecting === 'function') {
			// create function di Extender (jika perlu):
			// export async function obj_itemmanage_id_selecting(self, obj_itemmanage_id, frm, evt) {}
			fn_selecting(self, obj_itemmanage_id, frm, evt)
		} else {
			// default selecting
			const cbo = evt.detail.sender
			const dialog = evt.detail.dialog
			const searchtext = evt.detail.searchtext!=null ? evt.detail.searchtext : ''
			const url = 'itemmanage/header-list'
			const sort = {}
			const criteria = {
			...
			// export function obj_itemmanage_id_selecting_criteria(self, obj_itemmanage_id, frm, criteria, sort, evt) {}
			const fn_selecting_criteria_name = 'obj_itemmanage_id_selecting_criteria'
			const fn_selecting_criteria = Extender[fn_selecting_criteria_name]
			if (typeof fn_selecting_criteria === 'function') {
				fn_selecting_criteria(self, obj_itemmanage_id, frm, criteria, sort, evt)
			}
			...
			try {
				...
				for (var row of result.data) {
					evt.detail.addRow(row.itemmanage_id, row.itemmanage_name, row)
				}
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'itemclasstypeHeaderEdit_isEditDisabled'
		...
		// export async function itemclasstypeHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'itemclasstypeHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.itemclasstypeHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function itemclasstypeHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'itemclasstypeHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function itemclasstypeHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'itemclasstypeHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const itemclasstypeHeaderList = self.Modules.itemclasstypeHeaderList
	const listsecid = itemclasstypeHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'itemclasstypeHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function itemclasstypeHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.itemclasstypeHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'itemclasstypeHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function itemclasstypeHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'itemclasstypeHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'itemclasstypeHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function itemclasstypeHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.itemclasstypeHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.itemclasstypeHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.itemclasstypeHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.itemclasstypeHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.itemclasstypeHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'itemclasstypeHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About Item Class Type'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 9 Dec 2025 16:20'
