...
const CurrentSectionId = Context.Sections.taxtypeHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('taxtypeHeaderList-tbl')
const pnl_search = document.getElementById('taxtypeHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('taxtypeHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing taxtypeHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('taxtypeHeaderList-txt_searchtext')
		const btn_load = document.getElementById('taxtypeHeaderList-btn_gridload')
		...
		// taxtypeHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const taxtypeHeaderEdit = self.Modules.taxtypeHeaderEdit
		taxtypeHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const taxtypeHeaderEdit = self.Modules.taxtypeHeaderEdit
	taxtypeHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await taxtypeHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})
	} catch (err) {
		console.error(err)
	...
	setPagingButton(self, taxtypeHeaderEdit)
}
...
function tbl_cellclick(self, evt) {
	...
	const taxtypeHeaderEdit = self.Modules.taxtypeHeaderEdit
	taxtypeHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading taxtypeHeader data')
	console.log(params)
