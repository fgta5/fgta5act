import Context from './bccoaclass-context.mjs'
import * as Ext from './bccoaclass-ext.mjs'
...
const CurrentSectionId = Context.Sections.bccoaclassHeaderEdit
const CurrentSection = Crsl.Items[CurrentSectionId]
const Source = Context.Source


const TitleWhenNew = 'New BC COA Class'
const TitleWhenView = 'View BC COA Class'
const TitleWhenEdit = 'Edit BC COA Class'
const EditModeText = 'Edit'
const LockModeText = 'Lock'

const btn_edit = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_edit')
const btn_save = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_save')
const btn_new = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_new', 'bccoaclassHeader-new')
const btn_del = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_delete')
const btn_reset = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_reset')
const btn_prev = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_prev')
const btn_next = new $fgta5.ActionButton('bccoaclassHeaderEdit-btn_next')


const btn_recordstatus = document.getElementById('bccoaclassHeader-btn_recordstatus')
const btn_logs = document.getElementById('bccoaclassHeader-btn_logs')
const btn_about = document.getElementById('bccoaclassHeader-btn_about')

const frm = new $fgta5.Form('bccoaclassHeaderEdit-frm');
const obj_bccoaclass_id = frm.Inputs['bccoaclassHeaderEdit-obj_bccoaclass_id']
const obj_bccoaclass_name = frm.Inputs['bccoaclassHeaderEdit-obj_bccoaclass_name']	
...
export async function init(self, args) {
	console.log('initializing bccoaclassHeaderEdit ...')
...
export async function openSelectedData(self, params) {
	...
	try {
		...
		const fn_iseditdisabled_name = 'bccoaclassHeaderEdit_isEditDisabled'
		...
		// export async function bccoaclassHeaderEdit_formOpened(self, frm, CurrentState)
		const fn_formopened_name = 'bccoaclassHeaderEdit_formOpened'
...
async function backToList(self, evt) {
	...
	if (goback) {
		frm.lock()
		const listId =  Context.Sections.bccoaclassHeaderList
...
async function  frm_locked(self, evt) {
	...
	// export function bccoaclassHeaderEdit_formLocked(self, frm, CurrentState) {}
	const fn_name = 'bccoaclassHeaderEdit_formLocked'
...
async function  frm_unlocked(self, evt) {
	...
	// export function bccoaclassHeaderEdit_formUnlocked(self, frm, CurrentState) {}
	const fn_name = 'bccoaclassHeaderEdit_formUnlocked'
	const fn = Extender[fn_name]
	if (typeof fn === 'function') {
		fn(self, frm, CurrentState)
	}
...
async function btn_new_click(self, evt) {
	...
	const bccoaclassHeaderList = self.Modules.bccoaclassHeaderList
	const listsecid = bccoaclassHeaderList.Section.Id
	...
	try {
		...
		const fn_newdata_name = 'bccoaclassHeaderEdit_newData'
		const fn_newdata = Extender[fn_newdata_name]
		if (typeof fn_newdata === 'function') {
			// export async function bccoaclassHeaderEdit_newData(self, datainit, frm) {}
		...
		if (fromListSection) {
			...
			self.Modules.bccoaclassHeaderList.Section.show()
		}
	}
}
...
async function btn_save_click(self, evt) {
	...
	// Extender Autofill
	const fn_autofill_name = 'bccoaclassHeaderEdit_autofill'
	...
	// Extender Saving
	// export async function bccoaclassHeaderEdit_dataSaving(self, dataToSave, frm, args) {}
	const args = { cancelSave: false }
	const fn_datasaving_name = 'bccoaclassHeaderEdit_dataSaving'
	...
	try {
		...
		// Extender Saving
		const fn_datasaved_name = 'bccoaclassHeaderEdit_dataSaved'
		const fn_datasaved = Extender[fn_datasaved_name]
		if (typeof fn_datasaved === 'function') {
			// export async function bccoaclassHeaderEdit_dataSaved(self, data, frm) {}
		...
		if (isNewData) {
			...
			self.Modules.bccoaclassHeaderList.addNewRow(self, data)
		} else {
			console.log('update data baris yang dibuka')
			self.Modules.bccoaclassHeaderList.updateCurrentRow(self, data)
		}
...
async function btn_del_click(self, evt) {
	...
	try {
		...
		self.Modules.bccoaclassHeaderList.removeCurrentRow(self)
...
async function btn_prev_click(self, evt) {
	...
	self.Modules.bccoaclassHeaderList.selectPreviousRow(self)
}

async function btn_next_click(self, evt) {
	console.log('btn_next_click')
	self.Modules.bccoaclassHeaderList.selectNextRow(self)
}
...
async function btn_recordstatus_click(self, evt) {
	...
	pageHelper.openSection(self, 'fRecord-section', params, async ()=>{
		...
		try {
			...
			const fn_addrecordinfo_name = 'bccoaclassHeaderEdit_addRecordInfo'
...
async function btn_about_click(self, evt) {
	...
	pageHelper.openSection(self, 'fAbout-section', params, async ()=>{
		...
		AboutSection.Title = 'About BC COA Class'
		...
		if ( document.getElementById('fAbout-section-footer') == null) {
			...
			divFooter.innerHTML = 'This module is generated by fgta5 generator at 9 Dec 2025 19:56'
