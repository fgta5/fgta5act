...
const CurrentSectionId = Context.Sections.bccoaclassHeaderList
const CurrentSection = Crsl.Items[CurrentSectionId]
const CurrentState = {}

const tbl =  new $fgta5.Gridview('bccoaclassHeaderList-tbl')
const pnl_search = document.getElementById('bccoaclassHeaderList-pnl_search')
const btn_gridload = new $fgta5.ActionButton('bccoaclassHeaderList-btn_gridload') 

export const Section = CurrentSection
export const SearchParams = {}

export async function init(self, args) {
	console.log('initializing bccoaclassHeaderList ...')
	...
	try {
		...
		const obj_searchtext = document.getElementById('bccoaclassHeaderList-txt_searchtext')
		const btn_load = document.getElementById('bccoaclassHeaderList-btn_gridload')
		...
		// bccoaclassHeaderList-ext.mjs, export function initSearchParams(self, SearchParams) {} 
...
export function keyboardAction(self, actionName, evt) {
	if (actionName=='up') {
		...
		const bccoaclassHeaderEdit = self.Modules.bccoaclassHeaderEdit
		if (tbl.CurrentRow!=null) {
			bccoaclassHeaderEdit.Section.show({}, ()=>{
...
async function openRow(self, tr) {
	...
	const bccoaclassHeaderEdit = self.Modules.bccoaclassHeaderEdit
	bccoaclassHeaderEdit.clearForm(self, 'loading...')

	try {
		setCurrentRow(self, tr)
		CurrentState.SelectedRow.keyValue = keyvalue
		CurrentState.SelectedRow.key = key
		await bccoaclassHeaderEdit.openSelectedData(self, {key:key, keyvalue:keyvalue})

		// matikan atau nyalakan button prev/next sesuai kondisi
		setPagingButton(self, bccoaclassHeaderEdit)
...
function tbl_cellclick(self, evt) {
	...
	const bccoaclassHeaderEdit = self.Modules.bccoaclassHeaderEdit
	bccoaclassHeaderEdit.Section.show({}, (evt)=>{
		openRow(self, tr)
	})
...
async function tbl_loadData(self, params={}) {
	console.log('loading bccoaclassHeader data')
	console.log(params)
