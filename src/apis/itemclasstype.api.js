...
import * as Extender from './extenders/itemclasstype.apiext.js'

const moduleName = 'itemclasstype'
const headerSectionName = 'header'
const headerTableName = 'act.itemclasstype' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await itemclasstype_init(this, body) }

	// header
	async headerList(body) { return await itemclasstype_headerList(this, body) }
	async headerOpen(body) { return await itemclasstype_headerOpen(this, body) }
	async headerUpdate(body) { return await itemclasstype_headerUpdate(this, body)}
	async headerCreate(body) { return await itemclasstype_headerCreate(this, body)}
	async headerDelete(body) { return await itemclasstype_headerDelete(this, body) }
	
			
}	

// init module
async function itemclasstype_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.itemclasstype_init === 'function') {
			// export async function itemclasstype_init(self, initialData) {}
			await Extender.itemclasstype_init(self, initialData)
		}
...
// data logging
async function itemclasstype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function itemclasstype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `itemclasstype_name ILIKE '%' || \${searchtext} || '%'`,
	};
	...
	try {
		...
		for (var row of rows) {
			...
			// lookup: itemmanage_name dari field itemmanage_name pada table act.itemmanage dimana (act.itemmanage.itemmanage_id = act.itemclasstype.itemmanage_id)
			{
				const { itemmanage_name } = await sqlUtil.lookupdb(db, 'act.itemmanage', 'itemmanage_id', row.itemmanage_id)
				row.itemmanage_name = itemmanage_name
			}
...
async function itemclasstype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { itemclasstype_id: id }
		const searchMap = { itemclasstype_id: `itemclasstype_id = \${itemclasstype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
		...
		// lookup: itemmanage_name dari field itemmanage_name pada table act.itemmanage dimana (act.itemmanage.itemmanage_id = act.itemclasstype.itemmanage_id)
		{
			const { itemmanage_name } = await sqlUtil.lookupdb(db, 'act.itemmanage', 'itemmanage_id', data.itemmanage_id)
			data.itemmanage_name = itemmanage_name
		}
...
async function itemclasstype_headerCreate(self, body) {
	const { source='itemclasstype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			itemclasstype_log(self, body, startTime, tablename, ret.itemclasstype_id, 'CREATE', logMetadata)
...
async function itemclasstype_headerUpdate(self, body) {
	const { source='itemclasstype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['itemclasstype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			itemclasstype_log(self, body, startTime, tablename, data.itemclasstype_id, 'UPDATE')
...
async function itemclasstype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {itemclasstype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['itemclasstype_id'])
			...
			// record log
			itemclasstype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
