...
import * as Extender from './extenders/periode.apiext.js'

const moduleName = 'periode'
const headerSectionName = 'header'
const headerTableName = 'act.periode' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await periode_init(this, body) }

	// header
	async headerList(body) { return await periode_headerList(this, body) }
	async headerOpen(body) { return await periode_headerOpen(this, body) }
	async headerUpdate(body) { return await periode_headerUpdate(this, body)}
	async headerCreate(body) { return await periode_headerCreate(this, body)}
	async headerDelete(body) { return await periode_headerDelete(this, body) }
	
			
}	

// init module
async function periode_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.periode_init === 'function') {
			// export async function periode_init(self, initialData) {}
			await Extender.periode_init(self, initialData)
		}
...
// data logging
async function periode_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function periode_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `periode_id = \${searchtext} OR periode_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function periode_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { periode_id: id }
		const searchMap = { periode_id: `periode_id = \${periode_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function periode_headerCreate(self, body) {
	const { source='periode', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			periode_log(self, body, startTime, tablename, ret.periode_id, 'CREATE', logMetadata)
...
async function periode_headerUpdate(self, body) {
	const { source='periode', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['periode_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			periode_log(self, body, startTime, tablename, data.periode_id, 'UPDATE')
...
async function periode_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {periode_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['periode_id'])
			...
			// record log
			periode_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
