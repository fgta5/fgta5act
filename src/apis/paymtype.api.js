...
import * as Extender from './extenders/paymtype.apiext.js'

const moduleName = 'paymtype'
const headerSectionName = 'header'
const headerTableName = 'act.paymtype' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await paymtype_init(this, body) }

	// header
	async headerList(body) { return await paymtype_headerList(this, body) }
	async headerOpen(body) { return await paymtype_headerOpen(this, body) }
	async headerUpdate(body) { return await paymtype_headerUpdate(this, body)}
	async headerCreate(body) { return await paymtype_headerCreate(this, body)}
	async headerDelete(body) { return await paymtype_headerDelete(this, body) }
	
			
}	

// init module
async function paymtype_init(self, body) {
	const req = self.req
...
// data logging
async function paymtype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function paymtype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `paymtype_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function paymtype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { paymtype_id: id }
		const searchMap = { paymtype_id: `paymtype_id = \${paymtype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function paymtype_headerCreate(self, body) {
	const { source='paymtype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			paymtype_log(self, body, startTime, tablename, ret.paymtype_id, 'CREATE', logMetadata)
...
async function paymtype_headerUpdate(self, body) {
	const { source='paymtype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['paymtype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			paymtype_log(self, body, startTime, tablename, data.paymtype_id, 'UPDATE')
...
async function paymtype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {paymtype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['paymtype_id'])
			...
			// record log
			paymtype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
