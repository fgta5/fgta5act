...
import * as Extender from './extenders/bccycle.apiext.js'

const moduleName = 'bccycle'
const headerSectionName = 'header'
const headerTableName = 'act.bccycle' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await bccycle_init(this, body) }

	// header
	async headerList(body) { return await bccycle_headerList(this, body) }
	async headerOpen(body) { return await bccycle_headerOpen(this, body) }
	async headerUpdate(body) { return await bccycle_headerUpdate(this, body)}
	async headerCreate(body) { return await bccycle_headerCreate(this, body)}
	async headerDelete(body) { return await bccycle_headerDelete(this, body) }
	
			
}	

// init module
async function bccycle_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.bccycle_init === 'function') {
			// export async function bccycle_init(self, initialData) {}
			await Extender.bccycle_init(self, initialData)
		}
...
// data logging
async function bccycle_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function bccycle_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `bccycle_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function bccycle_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { bccycle_id: id }
		const searchMap = { bccycle_id: `bccycle_id = \${bccycle_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function bccycle_headerCreate(self, body) {
	const { source='bccycle', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			bccycle_log(self, body, startTime, tablename, ret.bccycle_id, 'CREATE', logMetadata)
...
async function bccycle_headerUpdate(self, body) {
	const { source='bccycle', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['bccycle_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			bccycle_log(self, body, startTime, tablename, data.bccycle_id, 'UPDATE')
...
async function bccycle_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {bccycle_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['bccycle_id'])
			...
			// record log
			bccycle_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
