...
import * as Extender from './extenders/taxtype.apiext.js'

const moduleName = 'taxtype'
const headerSectionName = 'header'
const headerTableName = 'act.taxtype' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await taxtype_init(this, body) }

	// header
	async headerList(body) { return await taxtype_headerList(this, body) }
	async headerOpen(body) { return await taxtype_headerOpen(this, body) }
	async headerUpdate(body) { return await taxtype_headerUpdate(this, body)}
	async headerCreate(body) { return await taxtype_headerCreate(this, body)}
	async headerDelete(body) { return await taxtype_headerDelete(this, body) }
	
			
}	

// init module
async function taxtype_init(self, body) {
	const req = self.req
...
// data logging
async function taxtype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function taxtype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `taxtype_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function taxtype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { taxtype_id: id }
		const searchMap = { taxtype_id: `taxtype_id = \${taxtype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function taxtype_headerCreate(self, body) {
	const { source='taxtype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			taxtype_log(self, body, startTime, tablename, ret.taxtype_id, 'CREATE', logMetadata)
...
async function taxtype_headerUpdate(self, body) {
	const { source='taxtype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['taxtype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			taxtype_log(self, body, startTime, tablename, data.taxtype_id, 'UPDATE')
...
async function taxtype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {taxtype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['taxtype_id'])
			...
			// record log
			taxtype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
