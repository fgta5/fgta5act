...
const headerTableName = 'act.coagroup' 	
...
async function coagroup_headerList(self, body) {
	...
	try {
		...
		for (var row of rows) {
			...
			// lookup: coagroup_parent_name dari field coagroup_name pada table act.coagroup dimana (act.coagroup.coagroup_id = act.coagroup.coagroup_parent)
			{
...
async function coagroup_headerCreate(self, body) {
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const cmd = sqlUtil.createInsertCommand(tablename, data, ['coagroup_id'])
