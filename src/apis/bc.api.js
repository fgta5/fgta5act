...
import * as Extender from './extenders/bc.apiext.js'

const moduleName = 'bc'
const headerSectionName = 'header'
const headerTableName = 'act.bc' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await bc_init(this, body) }

	// header
	async headerList(body) { return await bc_headerList(this, body) }
	async headerOpen(body) { return await bc_headerOpen(this, body) }
	async headerUpdate(body) { return await bc_headerUpdate(this, body)}
	async headerCreate(body) { return await bc_headerCreate(this, body)}
	async headerDelete(body) { return await bc_headerDelete(this, body) }
	
			
}	

// init module
async function bc_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.bc_init === 'function') {
			// export async function bc_init(self, initialData) {}
			await Extender.bc_init(self, initialData)
		}
...
// data logging
async function bc_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function bc_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `bc_doc = \${searchtext} OR bc_title ILIKE '%' || \${searchtext} || '%'`,
	};
	...
	try {
		...
		for (var row of rows) {
			...
			// lookup: dept_name dari field dept_name pada table ent.dept dimana (ent.dept.dept_id = act.bc.dept_id)
			{
				const { dept_name } = await sqlUtil.lookupdb(db, 'ent.dept', 'dept_id', row.dept_id)
				row.dept_name = dept_name
			}
			// lookup: bctype_name dari field bctype_name pada table act.bctype dimana (act.bctype.bctype_id = act.bc.bctype_id)
			{
				const { bctype_name } = await sqlUtil.lookupdb(db, 'act.bctype', 'bctype_id', row.bctype_id)
				row.bctype_name = bctype_name
			}
			// lookup: curr_code dari field curr_code pada table ent.curr dimana (ent.curr.curr_id = act.bc.curr_id)
			{
				const { curr_code } = await sqlUtil.lookupdb(db, 'ent.curr', 'curr_id', row.curr_id)
				row.curr_code = curr_code
			}
			// lookup: coa_name dari field coa_name pada table act.coa dimana (act.coa.coa_id = act.bc.coa_id)
			{
...
async function bc_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { bc_id: id }
		const searchMap = { bc_id: `bc_id = \${bc_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function bc_headerCreate(self, body) {
	const { source='bc', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			const args = { section: 'header', prefix:'BC' }
			...
			// generate data sesuai prefix (default:  BC) reset pertahun
			const seqdata = await sequencer.yearly(args.prefix)	
			data.bc_id = seqdata.id
			...
			// record log
			bc_log(self, body, startTime, tablename, ret.bc_id, 'CREATE', logMetadata)
...
async function bc_headerUpdate(self, body) {
	const { source='bc', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['bc_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			bc_log(self, body, startTime, tablename, data.bc_id, 'UPDATE')
...
async function bc_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['bc_id'])
			...
			// record log
			bc_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
