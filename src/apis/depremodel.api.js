...
import * as Extender from './extenders/depremodel.apiext.js'

const moduleName = 'depremodel'
const headerSectionName = 'header'
const headerTableName = 'act.depremodel' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await depremodel_init(this, body) }

	// header
	async headerList(body) { return await depremodel_headerList(this, body) }
	async headerOpen(body) { return await depremodel_headerOpen(this, body) }
	async headerUpdate(body) { return await depremodel_headerUpdate(this, body)}
	async headerCreate(body) { return await depremodel_headerCreate(this, body)}
	async headerDelete(body) { return await depremodel_headerDelete(this, body) }
	
			
}	

// init module
async function depremodel_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.depremodel_init === 'function') {
			// export async function depremodel_init(self, initialData) {}
			await Extender.depremodel_init(self, initialData)
		}
...
// data logging
async function depremodel_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function depremodel_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `depremodel_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function depremodel_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { depremodel_id: id }
		const searchMap = { depremodel_id: `depremodel_id = \${depremodel_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function depremodel_headerCreate(self, body) {
	const { source='depremodel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			depremodel_log(self, body, startTime, tablename, ret.depremodel_id, 'CREATE', logMetadata)
...
async function depremodel_headerUpdate(self, body) {
	const { source='depremodel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['depremodel_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			depremodel_log(self, body, startTime, tablename, data.depremodel_id, 'UPDATE')
...
async function depremodel_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {depremodel_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['depremodel_id'])
			...
			// record log
			depremodel_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
