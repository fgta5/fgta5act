...
import * as Extender from './extenders/bccoaclass.apiext.js'

const moduleName = 'bccoaclass'
const headerSectionName = 'header'
const headerTableName = 'act.bccoaclass' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await bccoaclass_init(this, body) }

	// header
	async headerList(body) { return await bccoaclass_headerList(this, body) }
	async headerOpen(body) { return await bccoaclass_headerOpen(this, body) }
	async headerUpdate(body) { return await bccoaclass_headerUpdate(this, body)}
	async headerCreate(body) { return await bccoaclass_headerCreate(this, body)}
	async headerDelete(body) { return await bccoaclass_headerDelete(this, body) }
	
			
}	

// init module
async function bccoaclass_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.bccoaclass_init === 'function') {
			// export async function bccoaclass_init(self, initialData) {}
			await Extender.bccoaclass_init(self, initialData)
		}
...
// data logging
async function bccoaclass_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function bccoaclass_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `bccoaclass_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function bccoaclass_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { bccoaclass_id: id }
		const searchMap = { bccoaclass_id: `bccoaclass_id = \${bccoaclass_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function bccoaclass_headerCreate(self, body) {
	const { source='bccoaclass', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			bccoaclass_log(self, body, startTime, tablename, ret.bccoaclass_id, 'CREATE', logMetadata)
...
async function bccoaclass_headerUpdate(self, body) {
	const { source='bccoaclass', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['bccoaclass_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			bccoaclass_log(self, body, startTime, tablename, data.bccoaclass_id, 'UPDATE')
...
async function bccoaclass_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {bccoaclass_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['bccoaclass_id'])
			...
			// record log
			bccoaclass_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
