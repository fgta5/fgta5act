...
import * as Extender from './extenders/itemmanage.apiext.js'

const moduleName = 'itemmanage'
const headerSectionName = 'header'
const headerTableName = 'act.itemmanage' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await itemmanage_init(this, body) }

	// header
	async headerList(body) { return await itemmanage_headerList(this, body) }
	async headerOpen(body) { return await itemmanage_headerOpen(this, body) }
	async headerUpdate(body) { return await itemmanage_headerUpdate(this, body)}
	async headerCreate(body) { return await itemmanage_headerCreate(this, body)}
	async headerDelete(body) { return await itemmanage_headerDelete(this, body) }
	
			
}	

// init module
async function itemmanage_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.itemmanage_init === 'function') {
			// export async function itemmanage_init(self, initialData) {}
			await Extender.itemmanage_init(self, initialData)
		}
...
// data logging
async function itemmanage_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function itemmanage_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `itemmanage_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function itemmanage_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { itemmanage_id: id }
		const searchMap = { itemmanage_id: `itemmanage_id = \${itemmanage_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function itemmanage_headerCreate(self, body) {
	const { source='itemmanage', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			itemmanage_log(self, body, startTime, tablename, ret.itemmanage_id, 'CREATE', logMetadata)
...
async function itemmanage_headerUpdate(self, body) {
	const { source='itemmanage', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['itemmanage_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			itemmanage_log(self, body, startTime, tablename, data.itemmanage_id, 'UPDATE')
...
async function itemmanage_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {itemmanage_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['itemmanage_id'])
			...
			// record log
			itemmanage_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
