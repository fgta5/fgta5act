...
import * as Extender from './extenders/agingtype.apiext.js'

const moduleName = 'agingtype'
const headerSectionName = 'header'
const headerTableName = 'act.agingtype' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await agingtype_init(this, body) }

	// header
	async headerList(body) { return await agingtype_headerList(this, body) }
	async headerOpen(body) { return await agingtype_headerOpen(this, body) }
	async headerUpdate(body) { return await agingtype_headerUpdate(this, body)}
	async headerCreate(body) { return await agingtype_headerCreate(this, body)}
	async headerDelete(body) { return await agingtype_headerDelete(this, body) }
	
			
}	

// init module
async function agingtype_init(self, body) {
	const req = self.req
...
// data logging
async function agingtype_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function agingtype_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `agingtype_name = \${searchtext}`,
	};
...
async function agingtype_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { agingtype_id: id }
		const searchMap = { agingtype_id: `agingtype_id = \${agingtype_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function agingtype_headerCreate(self, body) {
	const { source='agingtype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			agingtype_log(self, body, startTime, tablename, ret.agingtype_id, 'CREATE', logMetadata)
...
async function agingtype_headerUpdate(self, body) {
	const { source='agingtype', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['agingtype_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			agingtype_log(self, body, startTime, tablename, data.agingtype_id, 'UPDATE')
...
async function agingtype_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {agingtype_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['agingtype_id'])
			...
			// record log
			agingtype_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
