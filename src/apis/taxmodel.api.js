...
import * as Extender from './extenders/taxmodel.apiext.js'

const moduleName = 'taxmodel'
const headerSectionName = 'header'
const headerTableName = 'act.taxmodel' 	
...
export default class extends Api {
	...
	// contoh: header-list
	//         header-open-data
	async init(body) { return await taxmodel_init(this, body) }

	// header
	async headerList(body) { return await taxmodel_headerList(this, body) }
	async headerOpen(body) { return await taxmodel_headerOpen(this, body) }
	async headerUpdate(body) { return await taxmodel_headerUpdate(this, body)}
	async headerCreate(body) { return await taxmodel_headerCreate(this, body)}
	async headerDelete(body) { return await taxmodel_headerDelete(this, body) }
	
			
}	

// init module
async function taxmodel_init(self, body) {
	const req = self.req
	...
	try {
		...
		if (typeof Extender.taxmodel_init === 'function') {
			// export async function taxmodel_init(self, initialData) {}
			await Extender.taxmodel_init(self, initialData)
		}
...
// data logging
async function taxmodel_log(self, body, startTime, tablename, id, action, data={}, remark='') {
	const { source } = body
	const req = self.req
...
async function taxmodel_headerList(self, body) {
	const tablename = headerTableName
	const { criteria={}, limit=0, offset=0, columns=[], sort={} } = body
	const searchMap = {
		searchtext: `taxmodel_name ILIKE '%' || \${searchtext} || '%'`,
	};
...
async function taxmodel_headerOpen(self, body) {
	const tablename = headerTableName

	try {
		const { id } = body 
		const criteria = { taxmodel_id: id }
		const searchMap = { taxmodel_id: `taxmodel_id = \${taxmodel_id}`}
		const {whereClause, queryParams} = sqlUtil.createWhereClause(criteria, searchMap) 
...
async function taxmodel_headerCreate(self, body) {
	const { source='taxmodel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// record log
			taxmodel_log(self, body, startTime, tablename, ret.taxmodel_id, 'CREATE', logMetadata)
...
async function taxmodel_headerUpdate(self, body) {
	const { source='taxmodel', data={} } = body
	const req = self.req
	const user_id = req.session.user.userId
	const startTime = process.hrtime.bigint()
	...
	try {
		...
		const result = await db.tx(async tx=>{
			...
			// eksekusi update
			const cmd = sqlUtil.createUpdateCommand(tablename, data, ['taxmodel_id'])
			const ret = await cmd.execute(data)
			...
			// record log
			taxmodel_log(self, body, startTime, tablename, data.taxmodel_id, 'UPDATE')
...
async function taxmodel_headerDelete(self, body) {
	...
	try {
		...
		const deletedRow = await db.tx(async tx=>{
			...
			const dataToRemove = {taxmodel_id: id}
			...
			// hapus data header
			const cmd = sqlUtil.createDeleteCommand(tablename, ['taxmodel_id'])
			...
			// record log
			taxmodel_log(self, body, startTime, tablename, id, 'DELETE', logMetadata)
